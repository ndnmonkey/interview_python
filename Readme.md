...
self和cls是对类或者实例的绑定,对于一般的函数来说我们可以这么调用`foo(x)`,这个函数就是最常用的,它的工作跟任何东西(类,实例)无关.
对于实例方法,我们知道在类里每次定义方法的时候都需要绑定这个实例,就是`foo(self, x)`,为什么要这么做呢?因为实例方法的调用离不开实例,我们需要把实例自己传给函数,调用的时候是这样的`a.foo(x)`(其实是`foo(a, x)`).
类方法一样,只不过它传递的是类而不是实例,`A.class_foo(x)`.注意这里的self和cls可以替换别的参数,但是还是不要改的好.
...
>tcp的建立连接需要三次握手才能建立，而断开连接需要四次。
A：客户端；B：服务器（listen态）
第一次：
1. A->B，B知A具有发送能力。
2. B->A，A知B具有接受能力且有发送能力。（此时B还不知道A是否具有接受能力，因此需要第三次握手）
3. A->B,B知A具有接受能力。
>
